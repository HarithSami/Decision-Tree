...
def deviation(value,parent_pos,parent_neg):
    ...
    trainSet = SyntheticRestaurant(200)  
    tree = DecisionTreeLearner(trainSet)    # 3D (prune)
    testSet = SyntheticRestaurantPruneTest(1000)    #4A (evaluate)
    p_value, delta, error_rate = evaluate(tree,testSet) #evaluate P value
    clear_counts(tree)
    p_value,delta,error_rate = evaluate(tree,testSet)


      
    #deviation =(actual_positives - expected_positives)^2 /expected_positives +	                                        (actual_negatives - expected_negatives)^2 /expected_positives
...
def test_tennis_tree(filename):
    ...
    dataSet =  DataSet(name=filename, target='Play')
    trainSet,testSet = train_test_split(dataSet, test_split=0.8)
    tree = DecisionTreeLearner(dataSet)
    error = err_ratio(tree, dataSet)
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    dataSet =  DataSet(name=filename, target='Play')
    tree = DecisionTreeLearner(dataSet)
    samples_required=SyntheticRestaurantTest(199)
...
def train_tree(trainSet, testSet):
    ...
    trainSet = SyntheticRestaurant(200)  #3A
    testSet = SyntheticRestaurant(100)   #3B
